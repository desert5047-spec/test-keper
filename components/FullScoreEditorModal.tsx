...
export type FullScoreEditorModalProps = {
  visible: boolean;
  /** 左側に表示する点数（編集不可）。例: 50 */
  currentScore: number | null;
  /** 編集対象の満点の初期値。null のときは空で表示 */
...
const MAX_FULL_SCORE = 9999;
const MAX_LEN = 4;

export function FullScoreEditorModal({
  visible,
  currentScore,
  initialValue,
  onClose,
  ...
  useEffect(() => {
    if (visible) {
      if (initialValue == null || initialValue === 0) {
        ...
        const v = Math.min(MAX_FULL_SCORE, Math.max(1, initialValue));
        setDraft(String(v));
      }
    }
  ...
  const handleConfirm = () => {
    ...
    const value = Number.isFinite(parsed) ? Math.min(MAX_FULL_SCORE, Math.max(1, parsed)) : 100;
    onConfirm(value);
    onClose();
  };
      ...
      <Pressable style={styles.overlay} onPress={onClose}>
        <Pressable style={styles.content} onPress={(e) => e.stopPropagation()}>
          <View style={styles.header}>
            <Text style={styles.title}>満点を入力</Text>
          ...
          <View style={styles.displayRow}>
            ...
            <Text style={styles.displayDraft} numberOfLines={1}>
              {draft || '—'}
            </Text>
            <Text style={styles.displayUnit} numberOfLines={1}>
              {' 点中'}
          ...
          {draft !== '' && !isValid && (
            <Text style={styles.errorText}>
              {parsed < 1 || isNaN(parsed)
                ? '1以上を入力してください'
                : `${MAX_FULL_SCORE}以下で入力してください`}
            </Text>
          )}
