...
| 5 | **メモ入力（iOS）** | 記録追加画面でメモ欄をタップ → キーボード表示時、カーソルが枠外に飛ばず、入力可能であること |
| 6 | テキスト色 | 「ログイン情報を保存」「キャンセル」「メモ」等のテキストが白背景で黒系で見える（Android で白字にならない） |
| 7 | フォント・レイアウト | 日本語が正しく表示され、レイアウトが崩れていない |
